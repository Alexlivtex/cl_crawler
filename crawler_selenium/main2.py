...
def analysis_website():
    ...
    for driver_item in total_process_list:
        while True:
    ...
    for page_index in range(max_nocode_count):
        ...
        total_process_list[current_process_index].get(url)
        time.sleep(2)
        page_list_content = bs.BeautifulSoup(total_process_list[current_process_index].page_source).findAll("h3")
        for title_item in page_list_content:
            try:
                ...
                print(title)
                print(item_link)
                total_process_list[current_process_index].get(item_link)
                time.sleep(2)
                item_soup = bs.BeautifulSoup(total_process_list[current_process_index].page_source)
                title_content = item_soup.findAll("title")[0].text
                if title_content == "Attention Required! | Cloudflare":
                    time.sleep(10)
                    total_process_list[current_process_index].quit()
                ...
                for link_item in link_torrent:
                    if link_item.text[:21] == "http://www.rmdown.com":
                        break
                total_process_list[current_process_index].get(link_item.text)
                print(link_item.text)
                torrent_soup = bs.BeautifulSoup(total_process_list[current_process_index].page_source)
